    ...
    <div>
      ...
      {notes.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {notes.map((n) => (
            <div key={n.id} className={`card relative ${n.isPinned ? 'border-yellow-200 bg-yellow-50' : ''}`}>
              <div className="flex items-start justify-between gap-2 mb-2">
                <h3 className="font-semibold text-gray-900 text-sm">{n.title ?? 'Note'}</h3>
                <div className="flex gap-1 shrink-0">
                  <button
                    onClick={() => pinMutation.mutate(n.id)}
                    className={`p-1 transition-colors ${n.isPinned ? 'text-yellow-500' : 'text-gray-300 hover:text-yellow-400'}`}
                  >
                    <Pin size={14} />
                  </button>
                  <button
              ...
              <p className="text-sm text-gray-600 leading-relaxed line-clamp-4">{n.content}</p>
